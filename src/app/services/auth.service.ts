...
export class AuthService {
  ...
  URlmaster ='http://localhost:4000/api//photos/maestros'
  ...
  deleteuser(id: string) {
    return axios.post('http://localhost:4000/api/usersdelete'+`/${id}`);
  }

  updateuser(id: string, type: string) {
   // return axios.post('http://localhost:4000/api/usersupdate'+`/${id}`, {type});
    return this.http.put('http://localhost:4000/api/usersupdate'+`/${id}`, {type});
  }
